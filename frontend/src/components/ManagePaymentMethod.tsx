...
import { SelectChangeEvent } from "@mui/material/Select"; // ต้อง import ตัวนี้
import {
  Button,
  TextField,
  Select,
...
const ManagePaymentMethod: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedPaymentMethod) {
      const method = paymentMethods.find(
        (pm) => pm._id === selectedPaymentMethod,
      );
      if (method) {
        setFormData(method);
      }
  ...
  const handleMethodChange = (
    e: SelectChangeEvent<"credit_card" | "bank_account">,
  ) => {
    const method = e.target.value as "credit_card" | "bank_account";
    setFormData((prev) => ({
      ...prev,
      method,
      cardNumber: method === "credit_card" ? "" : prev.cardNumber,
      bankAccountNumber:
        method === "bank_account" ? "" : prev.bankAccountNumber,
      bankName: method === "bank_account" ? undefined : prev.bankName,
    }));
  };

  function sanitizeFormData(data: Partial<PaymentMethod>): PaymentMethod {
    return {
      _id: data._id ?? "",
      user: data.user ?? "",
      name: data.name ?? "",
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setPaymentMethods((prev) =>
        prev.filter((pm) => pm._id !== selectedPaymentMethod),
      );
  ...
  function isValidBankName(name: any): name is (typeof validBankNames)[number] {
