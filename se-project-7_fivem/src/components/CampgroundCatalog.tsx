...
export default function CampgroundCatalog({ campgroundsJson }: CampgroundCatalogProps) {
    ...
    const fetchCampgrounds = async () => {
        try {
            setLoading(true);
            if (campgroundsJson) {
                const result = await campgroundsJson;
                setData(result);
            ...
            <main className='relative py-12 px-4 sm:px-8 max-w-[1800px] mx-auto'>
                {count > 0 ? (
                    ...
                    <div className='text-center py-20 bg-white/90 backdrop-blur-sm rounded-xl shadow-lg border border-white/30 max-w-2xl mx-auto'>
                        ...
                        <h3 className='text-2xl font-semibold text-stone-800 mb-3'>No Campsites Found</h3>
                        <p className='text-stone-600 mb-6'>The wilderness is empty... for now</p>
