...
export default function CampgroundDetailPage({ params }: { params: { campid: string } }) {
  const [campgroundDetail, setCampgroundDetail] = useState<CampgroundResponse | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getCampground(params.campid);
        setCampgroundDetail(data);
      } catch (err: any) {
        setError(err.message || 'Error fetching campground details');
      }
    };
  ...
  // If data is loading
  if (!campgroundDetail) {
    return <div className="flex justify-center items-center min-h-screen bg-green-50">
