...
export default async function CampgroundCatalog({ campgroundsJson }: { campgroundsJson: Promise<CampgroundJson> }) {
    ...
    if (!success) return (
        <div className='py-12 px-6 bg-green-50 min-h-screen flex flex-col items-center justify-center'>
            <div className='bg-white p-8 rounded-xl shadow-lg border border-green-200 max-w-md text-center'>
                <Mountain className='h-12 w-12 text-green-600 mx-auto mb-4' />
                <h3 className='text-xl font-semibold text-green-800 mb-2'>Failed to load campgrounds</h3>
                <p className='text-green-600'>Please try refreshing the page or come back later</p>
            </div>
        </div>
    );

    return (
        <div className='py-12 px-6 bg-gradient-to-br from-green-50 to-green-100 min-h-screen'>
            ...
            {count === 0 && (
                <div className='text-center py-12'>
                    ...
                    <h3 className='text-xl font-medium text-green-800'>No campgrounds found</h3>
                    <p className='text-green-600 mt-2'>Check back later for new locations</p>
                </div>
            )}
        </div>
    );
